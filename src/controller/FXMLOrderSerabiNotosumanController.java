...
public class FXMLOrderSerabiNotosumanController implements Initializable {
  ...
  ProductData product = new ProductData("", 10000);
  ...
  void loadOrder() {
    ...
    try {
      fileInput = new FileInputStream("orderserabi.xml");
  ...
  void loadCheckout() {
    FileInputStream fileInput = null;
    try {
      fileInput = new FileInputStream("checkoutserabi.xml");
  ...
  void saveXML() {
    ...
    try {
      f = new FileOutputStream("orderserabi.xml");
  ...
  void saveXMLCO(ObservableList<CheckoutData> checkoutList) {
    ...
    FileOutputStream f = null;
    try {
      f = new FileOutputStream("checkoutserabi.xml");
      byte[] bytes = xml.getBytes("UTF-8");
      f.write(bytes);
    } catch (Exception e) {
      System.out.println("Warning: " + e.getMessage());
  ...
  public void initialize(URL location, ResourceBundle resource) {
    ...
    pilihanProduk.getItems().addAll("Serabi Original (isi 4)", "Serabi Coklat (isi 4)", "Serabi Campur (isi 4)");
    tujuan.getItems().addAll("Lokasi A", "Lokasi B", "Lokasi C", "Lokasi D", "Lokasi E");
  }
