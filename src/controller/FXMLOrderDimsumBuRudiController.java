...
public class FXMLOrderDimsumBuRudiController implements Initializable {
  ...
  void loadOrder() {
    ...
    try {
      fileInput = new FileInputStream("orderdimsum.xml");
  ...
  void loadCheckout() {
    FileInputStream fileInput = null;
    try {
      fileInput = new FileInputStream("checkoutdimsum.xml");
  ...
  void saveXML() {
    ...
    try {
      f = new FileOutputStream("orderdimsum.xml");
  ...
  void saveXMLCO(ObservableList<CheckoutData> checkoutList) {
    ...
    FileOutputStream f = null;
    try {
      f = new FileOutputStream("checkoutdimsum.xml");
      byte[] bytes = xml.getBytes("UTF-8");
      f.write(bytes);
    } catch (Exception e) {
      System.out.println("Warning: " + e.getMessage());
  ...
  public void initialize(URL location, ResourceBundle resource) {
    ...
    pilihanProduk.getItems().addAll("Siomay Original (isi 7)", "Siomay Mentai (isi 5)", "Bakpao Ayam (isi 4)", "Hakau (isi 7)");
